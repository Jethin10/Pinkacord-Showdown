...
export class PSStorage {
	...
	static init(): void | Promise<void> {
		...
		// Self-hosted: skip cross-origin setup when serving from the same host as the game server
		if (location.hostname === Config.defaultserver.host || location.hostname === 'localhost') {
			Config.server ||= Config.defaultserver;
			return;
		}
		if (`${location.protocol}//${location.hostname}` === PSStorage.origin) {
